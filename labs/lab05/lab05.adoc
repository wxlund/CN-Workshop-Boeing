...
1) Review the `lab05/service-registry/pom.xml` file. By adding `spring-cloud-starter-eureka-server` to the classpath this application is eligible to embed an Eureka server.
[source, xml]
<dependency>
...
`lab05/service-registry/src/main/java/io/pivotal/ServiceRegistryApplication.java`
...
3). Review the following file: `lab05/service-registry/src/main/resources/application.yml`
...
1) Review the `lab05/fortune-service/src/main/resources/bootstrap.yml` file. The name of this app is `fortune-service`. It also uses the `config-server`.
```yml
 server:
   port: 8787
 spring:
...
2) Review the `lab05/fortune-service/pom.xml` file. By adding spring-cloud-services-starter-service-registry to the classpath this application is eligible to register and discover services with the service-registry.
...
```
3) Review the following file: `lab05/fortune-service/src/main/java/io/pivotal/FortuneServiceApplication.java`. Notice the `@EnableDiscoveryClient`. This enables a discovery client that registers the fortune-service with the `service-registry` application.
...
1) Review the
`lab05/greeting-service/src/main/resources/bootstrap.yml` file. The name of this app is greeting-service. It also uses the config-server.
...
```
2) Review the `lab05/greeting-service/pom.xml` file. By adding `spring-cloud-services-starter-service-registry` to the classpath this application is eligible to register and discover services with the service-registry.
```xml
<dependency>
	<groupId>io.pivotal.spring.cloud</groupId>
	<artifactId>spring-cloud-services-starter-service-registry</artifactId>
...
3) Review the following file:
 `lab05/greeting-service/src/main/java/io/pivotal/GreetingServiceApplication.java`
...
4) Review the the following file:
 `lab05/greeting-service/src/main/java/io/pivotal/greeting/GreetingController.java`. Notice the `DiscoveryClient`. `DiscoveryClient` is used to discover services registered with the `service-registry`. See `fetchFortuneServiceUrl()`.
[source,java]
----
@Controller
...
`lab05/greeting-ribbon/src/main/java/io/pivotal/greeting/GreetingController.java`.
