...
. To simplify working with the endpoint for this lab, we will turn off its sensitivity. Add the following to $SPRING_BOOT_LABS_HOME/hello-spring-boot-actuator/src/main/resources/application.yml:
+
[source, yaml]
---------------------------------------------------------------------
info:
